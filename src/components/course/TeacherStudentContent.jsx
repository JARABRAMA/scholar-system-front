...
export function TeacherStudentContent() {
  return (
    <section className="flex flex-col gap-8 py-8 px-12">
      <header>
        <h1 className="text-4xl">Bienvenido a Scholar System</h1>
        <p className="text-stone-600">
          Tus grupos inscritos — horarios y profesores de este semestre
        </p>
      </header>
      <GroupsGrid />
    </section>
...
function ScheduleGroupCard({ schedules }) {
  return (
    <div className="flex flex-col p-4 gap-2 border-stone-300">
      ...
      {schedules.length === 0 && (
        <span className="self-center text-stone-600 p-2">
...
function GroupCardHeader({ groupName, courseName, groupId, credits }) {
  return (
    <div
      className="duration-300 grid grid-cols-[1fr_auto] gap-y-2 justify-between bg-sky-800 p-4 bg-sky-blue700
    ...
    >
      ...
      <span
        className="bg-sky-700 w-fit px-1.5 text-sm
       text-sky-200 font-bold justify-self-end rounded-full"
      >
        G-{formatTo4Digits(groupId)}
      </span>
      <span className="font-bold text-lg">
        {captalize(courseName.toLowerCase())}
      </span>
      <span className="text-sm justify-self-end text-blue-200">
        {credits} creditos
      </span>
    </div>
  );
